...
- `backend/monolith/order`: Spring Boot модуль API для checkout draft, supplementary checkout, получателя, адреса, доставки, оплаты, выгод, validation, резервирования, order confirmation и чтения результата заказа.
...
- Frontend вызывает `backend/monolith/order` по REST для `/api/order/checkouts`, `/api/order/checkouts/{checkoutId}`, `/api/order/checkouts/{checkoutId}/recipient`, `/address`, `/delivery`, `/payment`, `/benefits`, `/validation`, `/confirm` и `/api/order/orders/{orderNumber}`.
