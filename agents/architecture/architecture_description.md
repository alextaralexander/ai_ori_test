...
- `Order/Payment контур`: будущий контур оформления заказа, оплаты, дозаказа как order lifecycle и платежного handoff; feature #9 готовит валидированную корзину, но не создает order и payment.
...
- `cart` передает будущему `Order/Payment контуру` только валидированное состояние и checkout handoff; создание order, payment authorization и final bonus accrual не входят в feature #9.
